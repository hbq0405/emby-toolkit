...
def _tmdb_year_from_date(value) -> str:
    text = str(value or '').strip()
    return text[:4] if len(text) >= 4 and text[:4].isdigit() else ''


def _tmdb_summary_key(media_type: str, tmdb_id, season_number=None) -> str:
    mt = 'movie' if str(media_type or '').lower() == 'movie' else 'tv'
    sid = ''
    try:
        n = int(float(season_number))
        if n > 0:
            sid = str(n)
    except Exception:
        sid = ''
    return f"{mt}:{str(tmdb_id or '').strip()}:{sid}"


def _tmdb_get_json(path: str, api_key: str) -> dict:
    url = f"https://api.themoviedb.org/3/{str(path or '').strip().lstrip('/')}"
    resp = requests.get(url, params={'api_key': api_key, 'language': 'zh-CN'}, timeout=12)
...
def _tmdb_compact_summary(media_type: str, tmdb_id, detail: dict, season_number=None) -> dict:
    ...
    if mt == 'movie':
        title = detail.get('title') or detail.get('name') or detail.get('original_title') or detail.get('original_name') or ''
        year = _tmdb_year_from_date(detail.get('release_date') or detail.get('first_air_date'))
        poster_path = detail.get('poster_path') or ''
        backdrop_path = detail.get('backdrop_path') or ''
    else:
        title = detail.get('name') or detail.get('title') or detail.get('original_name') or detail.get('original_title') or ''
        year = _tmdb_year_from_date(detail.get('first_air_date') or detail.get('release_date'))
        poster_path = detail.get('poster_path') or ''
        backdrop_path = detail.get('backdrop_path') or ''
        season_item = None
        if season_no is not None:
            for item in detail.get('seasons') or []:
                if isinstance(item, dict) and item.get('season_number') == season_no:
                    season_item = item
                    break
        if season_item:
            poster_path = season_item.get('poster_path') or poster_path
            year = _tmdb_year_from_date(season_item.get('air_date')) or year

    genres = detail.get('genres') if isinstance(detail.get('genres'), list) else []
    return {
        'key': _tmdb_summary_key(mt, tmdb_id, season_no),
...
@any_login_required
def get_tmdb_summary_batch():
    """中心资源库海报墙批量取 TMDb 轻量元数据。"""
    payload = request.json or {}
    raw_items = payload.get('items') or payload.get('data') or []
    if not isinstance(raw_items, list):
        raw_items = []
    raw_items = raw_items[:120]

    api_key = config_manager.APP_CONFIG.get(constants.CONFIG_OPTION_TMDB_API_KEY)
    if not api_key:
        return jsonify({'success': False, 'message': '未配置 TMDb API Key', 'items': [], 'by_key': {}}), 400
    ...
    for raw in raw_items:
        if not isinstance(raw, dict):
            continue
        media_type = 'movie' if str(raw.get('media_type') or raw.get('item_type') or '').strip().lower() in {'movie', 'film', '电影'} else 'tv'
        tmdb_id = str(raw.get('tmdb_id') or raw.get('parent_series_tmdb_id') or raw.get('series_tmdb_id') or '').strip()
        if not tmdb_id:
            continue
        try:
            int(tmdb_id)
        ...
        try:
            if cache_key not in detail_cache:
                ...
                detail_cache[cache_key] = _tmdb_get_json(path, api_key)
            detail = detail_cache.get(cache_key) or {}
            if not detail:
                continue
            summary = _tmdb_compact_summary(media_type, tmdb_id, detail, season_number)
            by_key[summary['key']] = summary
        except Exception as e:
            errors.append({'media_type': media_type, 'tmdb_id': tmdb_id, 'error': str(e)})
