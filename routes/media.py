...
# ★★★ 演员纠错搜索API ★★★
@media_api_bp.route('/correct_actor_search', methods=['GET'])
@login_required
@processor_ready_required
def api_correct_actor_search():
    """
    根据演员名字搜索同名的其他演员，用于手动纠错。
    """
    actor_name = request.args.get('name', '').strip()
    if not actor_name:
        return jsonify({"error": "演员名字不能为空"}), 400

    try:
        # 调用新的 handler 函数
        search_results = emby_handler.search_emby_persons(
            base_url=extensions.media_processor_instance.emby_url,
            api_key=extensions.media_processor_instance.emby_api_key,
            user_id=extensions.media_processor_instance.emby_user_id,
        ...
        if search_results is None:
            return jsonify({"error": "在 Emby 中搜索演员时发生错误"}), 500
        ...
        for person in search_results:
            ...
            if person_id and image_tag:
                # 构造代理URL
                image_url = f"/image_proxy/Items/{person_id}/Images/Primary?tag={image_tag}&quality=90"

            formatted_results.append({
                "name": person.get("Name"),
                "emby_person_id": person_id,
                "production_year": person.get("ProductionYear"),
                "tmdbId": person.get("ProviderIds", {}).get("Tmdb"),
                "imageUrl": image_url
            })
        ...
        logger.error(f"API /api/correct_actor_search Error: {e}", exc_info=True)
