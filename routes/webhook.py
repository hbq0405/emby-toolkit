...
def _maybe_auto_promote_virtual_from_playback(data: dict, event_type: str, item_id: str, item_type: str, user_id: str):
    ...
    if event_type in USER_DATA_EVENTS:
        ...
        try:
            if len(update_data) > 2:
                ...
                try:
                    _maybe_auto_promote_virtual_from_playback(data, event_type, item_id_from_webhook, item_type_from_webhook, user_id)
                except Exception as e:
                    logger.warning(f"  ➜ [共享虚拟转正] Webhook 自动转正判断失败: {e}", exc_info=True)
