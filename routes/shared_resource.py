...
def _candidate_matches_share_request_target(candidate: Dict[str, Any], request_filter: Dict[str, Any]) -> bool:
    ...
    if target_type == 'episode':
        # 求单集也以所属季目录响应，不再返回/创建单集共享源。
        if cand_type not in {'Season', 'Series'} and str(candidate.get('share_type') or '').lower() != 'season_pack':
            return False
        return req_season is None or cand_season == req_season
...
def _expand_share_candidates_for_share_request(row: Dict[str, Any], request_filter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if target_type == 'episode':
        ...
        if ep_row:
            return [_build_media_candidate(_load_season_row_for_episode(ep_row))]
        # 找不到精确 Episode 行时，也让普通展开逻辑把命中的 Episode 上提为 Season。
...
def _share_policy_for_media(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if item_type == 'Movie':
        return {'allowed': True, 'share_type': 'movie_folder', 'share_item_type': 'Movie', 'message': '电影按目录共享，目录变更后更新共享索引'}
    if item_type == 'Episode':
        info = _season_completion_info({**(row or {}), 'item_type': 'Season'})
        return {'allowed': True, 'share_type': 'season_pack', 'share_item_type': 'Season', 'message': '单集不再作为共享源，已上提为所属季目录共享', 'completion': info}
    if item_type == 'Season':
        ...
        return {'allowed': True, 'share_type': 'season_pack', 'share_item_type': 'Season', 'message': '连载季按季目录共享，不做完结季一致性校验；目录变化后更新共享索引', 'completion': info}
    if item_type == 'Series':
        return {'allowed': False, 'share_type': 'season_pack', 'share_item_type': 'Season', 'message': '不直接共享整剧；请按季目录登记'}
...
def _resolve_share_root(media_row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if item_type == 'Movie' and len(file_rows) == 1:
        # Rapid v2 最小共享粒度改为电影目录：监控目录变化，后续可更新共享索引。
        parent_id = str(file_rows[0].get('parent_id') or '')
        if parent_id:
            root_id = parent_id
            root_node = _get_p115_node(root_id) or {}
            root_name = root_node.get('name') or root_id
            ...
            messages.append('电影已上提到父目录登记，后续目录文件变化可更新共享索引')
        else:
            root_id = str(file_rows[0].get('id') or '')
            root_is_dir = False
            share_type = 'movie_file'
            root_name = file_rows[0].get('name') or root_id
            messages.append('未找到电影父目录，只能临时按文件登记')
    elif item_type == 'Episode':
        # 单集不再作为共享源；上提到所属季目录。
        item_type = 'Season'
        ...
        if parent_ids:
            ...
            if narrowed.get('root_id'):
                root_id = narrowed.get('root_id')
                root_name = narrowed.get('root_name') or root_id
            root_is_dir = True
            messages.append('单集命中已上提为所属季目录共享')
        else:
            root_id = str(file_rows[0].get('id') or '')
            root_is_dir = False
            root_name = file_rows[0].get('name') or root_id
            messages.append('单集命中但未找到父目录，已临时保留文件定位；建议同步 p115_filesystem_cache 后重新登记季目录')
    elif parent_ids:
        chains = [_ancestor_chain(pid) for pid in parent_ids]
        common = []
        if chains:
...
def _load_season_row_for_episode(row: Dict[str, Any]) -> Dict[str, Any]:
    """Episode 命中时统一上提到所属 Season；找不到 Season 行时构造虚拟季行。"""
    row = dict(row or {})
    parent_series_id = str(row.get('parent_series_tmdb_id') or row.get('series_tmdb_id') or '').strip()
    season_number = row.get('season_number')
    if not parent_series_id or season_number in (None, ''):
        return row
    try:
        season_number_int = int(float(season_number))
    except Exception:
        return row
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    ...
                    ORDER BY tmdb_id
                    LIMIT 1
                """, (parent_series_id, season_number_int))
                season_row = cur.fetchone()
                if season_row:
                    return dict(season_row)
    except Exception as e:
        logger.debug(f"  ➜ [共享资源] 查询单集所属季失败: series={parent_series_id}, season={season_number_int}, err={e}")

    # 兜底虚拟 Season 行，_collect_media_identifiers 可用 parent_series_tmdb_id + season_number 下钻 Episode。
    return {
        ...
        'file_sha1_json': None,
        'file_pickcode_json': None,
        'in_library': True,
        'subscription_status': row.get('subscription_status'),
        'total_episodes': row.get('total_episodes'),
        'watching_status': row.get('watching_status'),
        'watchlist_tmdb_status': row.get('watchlist_tmdb_status'),
        'force_ended': row.get('force_ended'),
    }

def _expand_share_candidates(row: Dict[str, Any]) -> List[Dict[str, Any]]:
    """搜索结果展开策略：共享最小粒度固定为电影目录 / 季目录。"""
    row = dict(row or {})
    item_type = row.get('item_type')
    if item_type == 'Movie':
        return [_build_media_candidate(row)]
    if item_type == 'Episode':
        return [_build_media_candidate(_load_season_row_for_episode(row))]
    if item_type == 'Season':
        return [_build_media_candidate(row)]
    if item_type == 'Series':
        out = []
        ...
        if seasons:
            for season in seasons:
                out.append(_build_media_candidate(season))
                if len(out) >= 100:
                    break
        else:
            # 没有 Season 行时，用已有 Episode 反推季目录候选，但不返回单集候选。
            seen_seasons = set()
            episodes = _get_episode_rows_for_media(row, only_with_files=True)
            for ep in episodes[:300]:
                season_row = _load_season_row_for_episode(ep)
                key = (season_row.get('parent_series_tmdb_id') or season_row.get('tmdb_id'), season_row.get('season_number'))
                if key in seen_seasons:
                    continue
                seen_seasons.add(key)
                out.append(_build_media_candidate(season_row))
                if len(out) >= 100:
                    break
        if not out:
            disabled = _build_media_candidate(row)
            disabled['resolvable'] = False
            disabled['message'] = '不直接共享整剧；未找到可共享的季目录'
            out.append(disabled)
        return out
    return [_build_media_candidate(row)]
...
def api_shared_resource_config():
    ...
    if isinstance(payload, dict):
        ...
        payload.setdefault('p115_shared_block_clean_version_transfer', block_clean)
    try:
        from tasks.shared_resource_tasks import ensure_shared_device_event_listener, stop_shared_device_event_listener
        if _boolish(payload.get('p115_shared_resource_enabled') if isinstance(payload, dict) else data.get('p115_shared_resource_enabled'), False):
            ensure_shared_device_event_listener()
        else:
            stop_shared_device_event_listener()
    except Exception as e:
        logger.warning(f"  ➜ [共享资源] 保存配置后刷新共享事件监听失败: {e}")
...
def _manual_share_business_payload(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Rapid v2 的共享最小粒度收口为：电影目录 / 剧集季目录。
    单集不再作为可登记共享源；响应单集求共享时，也上提为该集所属季目录，
    这样后续目录变化才能通过季容器持续追踪和更新 manifest。
    """
    data = dict(data or {})
    share_request_payload = data.get('share_request_payload') if isinstance(data.get('share_request_payload'), dict) else {}
    request_target = str((share_request_payload or {}).get('target_type') or '').strip().lower()

    if request_target == 'episode':
        data['share_type'] = 'season_pack'
        data['item_type'] = 'Season'
        if (share_request_payload or {}).get('season_number') not in (None, ''):
            data['season_number'] = share_request_payload.get('season_number')
        # 保留求共享原始 episode_number 在 share_request_payload 中用于日志/悬赏闭环，
        # 但共享源本身不再登记 episode_number。
        data['episode_number'] = None

    share_type = str(data.get('share_type') or '').strip()
    item_type = str(data.get('item_type') or '').strip()

    if share_type == 'episode_file' or item_type == 'Episode':
        # 历史前端/旧缓存可能还传 episode_file，这里统一上提为季。
        data['share_type'] = 'season_pack'
        data['item_type'] = 'Season'
        data['episode_number'] = None
        share_type = 'season_pack'
    ...
    if share_type == 'series_pack':
        if request_target not in ('series', 'tv'):
            return {'ok': False, 'message': '普通手动登记仍禁用整剧共享；请选择电影目录或季目录', 'data': data}
    ...
    if share_type == 'season_pack':
        # 连载季也允许登记为季目录；只有完结季后续才做一致性校验。
        data['item_type'] = 'Season'
        data['episode_number'] = None

    if share_type in ('movie_file', 'movie_folder'):
        data['share_type'] = 'movie_folder'
...
def _season_share_requires_consistency(data: Dict[str, Any]) -> bool:
    """只有真实完结季包才做版本一致性校验；连载季目录只做 RAW/目录校验。"""
    data = dict(data or {})
    if str(data.get('share_type') or '').strip().lower() != 'season_pack':
        return False
    try:
        info = _season_completion_info({
            'item_type': 'Season',
...
def api_manual_validate_share():
    ...
    if share_type == 'season_pack' and _season_share_requires_consistency(share_data):
        consistency = _validate_season_pack_consistency(files, share_data)
        result['season_pack_consistency'] = consistency
        if not consistency.get('ok'):
            result['message'] = consistency.get('message') or '完结季包媒体参数不一致，禁止登记共享'
            return jsonify({'success': True, 'message': result['message'], 'data': result})
        result['valid'] = True
        result['message'] = consistency.get('message') or f"完结季包一致性校验通过：共 {len(files)} 个视频文件，可登记共享"
        return jsonify({'success': True, 'message': result['message'], 'data': result})

    result['valid'] = True
    if share_type == 'season_pack':
        result['message'] = f"连载季目录预校验通过：共 {len(files)} 个视频文件；不做完结季一致性校验，后续按目录变化更新共享。"
    else:
        result['message'] = f"预校验通过：共 {len(files)} 个视频文件，可登记共享"
    return jsonify({'success': True, 'message': result['message'], 'data': result})
...
def api_manual_create_share():
    ...
    normalized = _manual_share_business_payload(raw_data)
    if not normalized.get('ok'):
        return jsonify({"success": False, "message": normalized.get('message') or '共享源参数无效'}), 400
    data = normalized.get('data') or raw_data
    share_request_payload = data.get('share_request_payload') if isinstance(data.get('share_request_payload'), dict) else {}
    ...
    if str(data.get('share_type') or '').strip().lower() == 'season_pack' and _season_share_requires_consistency(data):
        consistency = _validate_season_pack_consistency(files, data)
        if not consistency.get('ok'):
            return jsonify({
                "success": False,
                "message": consistency.get('message') or "完结季包媒体参数不一致，禁止登记共享",
...
def api_register_center_device():
    ...
    try:
        ...
        try:
            from tasks.shared_resource_tasks import ensure_shared_device_event_listener
            ensure_shared_device_event_listener()
        except Exception as e:
            logger.warning(f"  ➜ [共享资源] 注册设备后启动共享事件监听失败: {e}")
...
def _load_center_sources_for_display(client, *, keyword: str = '', tmdb_id: str = '', display_type: str = '', status: str = CENTER_DISPLAY_SOURCE_STATUSES, order_by: str = 'latest', limit: int = 30, offset: int = 0, local_filter: str = '') -> Dict[str, Any]:
    ...
    # Rapid v2 展示分页/搜索/聚合必须由中心端完成；旧中心没有 display-list 时不再
    # 在客户端拉几千条 source 做本地折叠，避免中心资源库页面卡死。
    raise RuntimeError('中心端版本过低：缺少 /api/v1/sources/display-list，请先升级共享中心后再打开中心资源库。')
