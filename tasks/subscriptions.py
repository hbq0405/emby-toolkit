...
def _try_download_from_hdhive_first(tmdb_id, media_type, title, item_label="媒体", target_season=None):
    """
    统一的影巢优先处理：
    ...
    try:
        ...
        def _resource_score(r):
            ...
            # 剧集优先保季号：明确命中目标季 > 范围/合集 > 未标季；再按积分、115、体积排序。
            return (
                -season_level,
                effective_points,
        ...
        season_match_info = ""
        if media_type == "tv" and target_season is not None:
            season_match_info = f", 季匹配: {target_resource.get('_season_match_label') or '未知'}"
