...
def _subscription_consume_statuses() -> List[str]:
    """订阅资源自动消费状态：用户明确想要/订阅/请求/暂停的都应尝试从中心消费。"""
    return ['WANTED', 'SUBSCRIBED', 'REQUESTED', 'PAUSED']


def _subscribed_movies_and_seasons(limit: int = 80) -> List[Dict[str, Any]]:
    """查询需要从中心消费的电影和季。只查 Movie / Season，不查 Series。"""
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                WITH candidates AS (
                    SELECT DISTINCT ON (
                        CASE WHEN item_type='Movie' THEN tmdb_id ELSE parent_series_tmdb_id END,
                        item_type,
                ...
                LIMIT %s
                """,
                (_subscription_consume_statuses(), int(limit)),
            )
            return [dict(r) for r in cur.fetchall()]


def _subscribed_missing_episodes_for_season(season_row: Dict[str, Any], limit: int = 80) -> List[Dict[str, Any]]:
    """季包未命中时，展开该季尚未入库的分集，逐集从中心消费。"""
    parent = str(season_row.get('parent_series_tmdb_id') or season_row.get('tmdb_id') or '').strip()
    season = _safe_int(season_row.get('season_number'), -1)
    if not parent or season < 0:
        return []
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT DISTINCT ON (parent_series_tmdb_id, season_number, episode_number)
                    tmdb_id,
                    item_type,
                    parent_series_tmdb_id,
                    season_number,
                ...
                WHERE item_type='Episode'
                  ...
                  AND COALESCE(subscription_status, 'NONE') <> 'IGNORED'
                ...
                LIMIT %s
                """,
                (parent, season, int(limit)),
            )
            return [dict(r) for r in cur.fetchall()]


def _shared_source_is_season_pack(src: Dict[str, Any]) -> bool:
    """Season 首轮消费只允许季包源，不允许 Episode 单集源混进来。"""
    item_type = str(src.get('item_type') or '').strip().lower()
    episode_number = src.get('episode_number')
    return item_type in ('season', 'pack', 'series') and episode_number in (None, '', 0)


def _has_local_virtual_projection_for_movie(row: Dict[str, Any]) -> bool:
...
def _has_local_virtual_projection_for_season(row: Dict[str, Any]) -> bool:
    parent = str(row.get('parent_series_tmdb_id') or row.get('tmdb_id') or '').strip()
    season = _safe_int(row.get('season_number'), -1)
    if not parent or season < 0:
        return False
    try:
...
def _auto_consume_subscribed_movies_and_seasons_from_center(max_items: int = 80) -> Dict[str, int]:
    """从中心资源库消费 Movie / Season 订阅。Season 优先季包，季包未命中再逐集消费。"""
    if not _enabled():
        return {
            'missing': 0,
            'consumed': 0,
            'season_pack_consumed': 0,
            'episode_consumed': 0,
            'gaps': 0,
            'skipped': 0,
        }
    ...
    try:
        ...
        logger.warning(f"  ➜ [共享资源维护] 无法加载共享消费入口，跳过订阅资源消费: {e}")
        return {
            'missing': 0,
            'consumed': 0,
            'season_pack_consumed': 0,
    ...
    rows = _subscribed_movies_and_seasons(limit=max_items)
    consumed = season_pack_consumed = episode_consumed = gaps = skipped = 0
    consecutive_errors = 0
    ...
    for row in rows:
        try:
            ...
            if item_type == 'Movie':
                if _has_local_virtual_projection_for_movie(row):
                    skipped += 1
                    continue
                ...
                if result.get('success'):
                    consumed += 1
                    logger.info(
                        "  ➜ [共享资源维护] 电影订阅命中中心资源并已%s：%s",
                        '虚拟入库' if result.get('mode') == 'virtual' else '永久转存',
                        title or row.get('tmdb_id'),
                    )
                elif result.get('reported_gap'):
                    gaps += 1

            elif item_type == 'Season':
                parent_tmdb = row.get('parent_series_tmdb_id') or row.get('tmdb_id')
                season_number = row.get('season_number')
                season_title = title or f"{parent_tmdb} S{_safe_int(season_number, 0):02d}"

                if _has_local_virtual_projection_for_season(row):
                    skipped += 1
                    continue

                # 第一轮：只允许真正的季包源。这里不登记缺口，避免“无季包但有单集”时重复登记 Season 缺口。
                pack_result = try_consume_shared_resource(
                    row,
                    title=season_title,
                    tmdb_id=parent_tmdb,
                    item_type='Season',
                    ...
                    year=year,
                    source_filter=_shared_source_is_season_pack,
                    report_gap_when_empty=False,
                )

                if pack_result.get('success'):
                    consumed += 1
                    season_pack_consumed += 1
                    logger.info(
                        "  ➜ [共享资源维护] 季订阅优先命中季包并已%s：%s S%02d",
                        '虚拟入库' if pack_result.get('mode') == 'virtual' else '永久转存',
                        season_title,
                        _safe_int(season_number, 0),
                    )
                    continue

                # 第二轮：没有季包，再展开该季缺失分集逐集消费。
                episode_rows = _subscribed_missing_episodes_for_season(row, limit=120)
                if not episode_rows:
                    gaps += 1
                    continue

                any_episode_ok = False
                for ep in episode_rows:
                    try:
                        if _has_local_virtual_projection_for_episode(ep):
                            skipped += 1
                            continue

                        ep_title = ep.get('title') or season_title or f"S{_safe_int(ep.get('season_number'), 0):02d}E{_safe_int(ep.get('episode_number'), 0):02d}"
                        ep_result = try_consume_shared_resource(
                            ep,
                            title=ep_title,
                            tmdb_id=ep.get('tmdb_id') or parent_tmdb,
                            item_type='Episode',
                            parent_tmdb_id=parent_tmdb,
                            season_number=ep.get('season_number'),
                            year=ep.get('release_year') or year,
                        )
                        if ep_result.get('success'):
                            ...
                            any_episode_ok = True
                            logger.info(
                                "  ➜ [共享资源维护] 季包未命中，单集命中中心资源并已%s：%s S%02dE%02d",
                                '虚拟入库' if ep_result.get('mode') == 'virtual' else '永久转存',
                                season_title,
                                _safe_int(ep.get('season_number'), 0),
                                _safe_int(ep.get('episode_number'), 0),
                            )
                        elif ep_result.get('reported_gap'):
                            gaps += 1
                    except Exception as ep_e:
                        logger.warning(f"  ➜ [共享资源维护] 季订阅单集兜底消费失败: {ep} -> {ep_e}", exc_info=True)
                        if _is_network_error(ep_e):
                            consecutive_errors += 1
                            if consecutive_errors >= 3:
                                logger.error("  ➜ [共享资源维护] 连续 3 次网络请求失败，触发熔断，提前结束订阅资源消费。")
                                break
                    time.sleep(0.2)
            ...
            logger.warning(f"  ➜ [共享资源维护] 订阅资源共享消费失败: {row} -> {e}", exc_info=True)
            if _is_network_error(e):
                consecutive_errors += 1
                if consecutive_errors >= 3:
                    logger.error("  ➜ [共享资源维护] 连续 3 次网络请求失败，触发熔断，提前结束订阅资源消费。")
    ...
    return {
        'missing': len(rows),
        'consumed': consumed,
        'season_pack_consumed': season_pack_consumed,
...
def task_shared_resource_maintenance(processor=None, maintenance_silent: bool = False):
    ...
    try:
        ...
        _status(55, '正在从中心资源库处理电影/季订阅...')
        sub_result = _auto_consume_subscribed_movies_and_seasons_from_center()
        total.update({f'sub_{k}': v for k, v in sub_result.items()})
        ...
        msg = (
            ...
            f"订阅消费 {total.get('sub_consumed', 0)}/{total.get('sub_missing', 0)}，"
            f"季包 {total.get('sub_season_pack_consumed', 0)}，单集 {total.get('sub_episode_consumed', 0)}，"
