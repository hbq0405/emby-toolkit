...
from routes.shared_resource import shared_resource_bp
# --- 核心模块导入 ---
...
def _current_license_tier_hint() -> str:
    for key in ('pro_license_key', 'license_key'):
        try:
            value = settings_db.get_setting(key)
            tier = _infer_pro_tier_from_license(value)
            if tier:
                return tier
        except Exception:
            pass
    try:
        value = (config_manager.APP_CONFIG or {}).get('pro_license_key') or ''
        return _infer_pro_tier_from_license(value)
    except Exception:
        return ''
...
def _refresh_shared_device_event_listener(reason: str = "startup"):
    ...
    共享事件监听是常驻基础服务，但必须在设备 Token 存在时才启动。
    Token 丢失时如果仍然启动监听，后台线程会反复向中心发起无认证请求；
    在部分网络环境下还会把启动流程卡在中心请求/重试链路上，导致页面迟迟打不开。
    """
    try:
        ...
        center_url = str(cfg.get('p115_shared_center_url') or '').strip()
        ...
        if not device_token:
            stop_shared_device_event_listener()
            logger.warning(
                f"  ➜ [共享资源] 已启用但设备 Token 缺失，跳过中心事件监听启动 ({reason})；"
                "请在共享资源配置中点击“恢复设备 Token”。"
            )
            return

        if not center_url:
            stop_shared_device_event_listener()
            logger.warning(f"  ➜ [共享资源] 已启用但中心地址为空，跳过中心事件监听启动 ({reason})。")
            return
...
def initialize_processors():
    ...
    if emby_url and emby_key:
        ...
        config_manager.APP_CONFIG['pro_expire_time'] = ''
        ...
        if server_id_local:
            ...
            try:
                ...
                if resp.get("success") and resp.get("is_pro"):
                    tier = _normalize_pro_tier(
                        resp.get("tier") or resp.get("pro_tier") or resp.get("level") or resp.get("card_type")
                    ) or _current_license_tier_hint()
                    ...
                    logger.info(f"  ➜ Pro 验证通过！已解锁全部功能。等级={tier or '-'}")
                else:
                    config_manager.APP_CONFIG['is_pro_active'] = False
                    config_manager.APP_CONFIG['pro_tier'] = ''
                    config_manager.APP_CONFIG['pro_expire_time'] = ''
