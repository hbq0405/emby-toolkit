...
const centerRapidSourceId = (row) => {
  if (centerIsLogicalSeasonRow(row)) {
    const gid = centerLogicalGroupId(row);
    if (gid) return gid;
  }
  return String(
...
const buildCenterImportSourcePayload = (row) => {
  ...
  return {
    ...
    source_ref_id: sourceKind === 'logical_season' ? sourceId : (row?.source_ref_id || sourceId),
    logical_group_id: sourceKind === 'logical_season' ? sourceId : (row?.logical_group_id || row?.logical_group?.group_id || ''),
    group_id: sourceKind === 'logical_season' ? sourceId : row?.group_id,
    title: row?.title || row?.logical_group?.title || '',
