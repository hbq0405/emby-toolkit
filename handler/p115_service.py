...
class SmartOrganizer:
    ...
    def execute(self, root_item_or_items, target_cid, progress_callback=None, skip_gc=False):
        ...
        # ★★★ 智能物理时长嗅探 (解决短剧 TMDb 缺乏时长元数据的问题) ★★★
        # =================================================================
        # 1. 检查是否有启用的规则依赖了时长条件 (避免无意义的 ffprobe 消耗)
        # needs_runtime_check = any((r.get('runtime_min') or r.get('runtime_max')) for r in self.rules if r.get('enabled', True))
        ...
        #     # 2. 如果 TMDb 没有提供时长，我们去物理文件里抓
        #     if current_runtime == 0:
        #         known_video_exts_sniff = {'mp4', 'mkv', 'avi', 'ts', 'iso', 'rmvb', 'wmv', 'mov', 'm2ts', 'flv', 'mpg'}
        #         # 抓取第一个视频文件作为样本
        #         first_video = next((c for c in candidates if c.get('fn', c.get('n', c.get('file_name', ''))).split('.')[-1].lower() in known_video_exts_sniff), None)
