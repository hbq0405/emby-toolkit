            ...
            if not should_skip_full_processing:
                ...
                if self.ai_translator and self.config.get(constants.CONFIG_OPTION_AI_TRANSLATE_TITLE_OVERVIEW, False):
                    ...
                    if local_trans and local_trans.get('overview') and utils.contains_chinese(local_trans['overview']):
                        ...
                        current_overview = local_trans['overview'] # 更新变量以便下方判断
        ...
        def _extract_common_json_fields(details: Dict[str, Any], m_type: str):
            ...
            for g in genres_raw:
                if isinstance(g, dict): 
                    name = g.get('name')
                    if name in utils.GENRE_TRANSLATION_PATCH: name = utils.GENRE_TRANSLATION_PATCH[name]
            ...
            raw_studios = []
            if m_type == 'Series': raw_studios = details.get('networks') or []
            else: raw_studios = details.get('production_companies') or []
            ...
            for s in raw_studios:
                ...
                elif isinstance(s, str) and s: unique_studios_map[s] = {'id': None, 'name': s}
            ...
            elif isinstance(keywords_data, list): raw_k_list = keywords_data
            keywords = []
            for k in raw_k_list:
        ...
        try:
            ...
            # ==================================================================
                ...
                for season in seasons_details:
                    ...
                    if not s_tmdb_id or str(s_tmdb_id) in ['0', 'None', '']: continue
                ...
                for episode in episodes_details:
                    ...
                    if not is_pending and versions_of_episode:
                        ...
                        for v in versions_of_episode:
                            # 补全 SourceLibraryId
                            if not v.get('_SourceLibraryId'):
                                v['_SourceLibraryId'] = item_details_from_emby.get('_SourceLibraryId')
                            
                            # 解析资产详情 (现在 v 包含 MediaSources，能解析出正确数据)
    ...
    def _process_item_core_logic(self, item_details_from_emby: Dict[str, Any], force_full_update: bool = False, specific_episode_ids: Optional[List[str]] = None):
        ...
        try:
            ...
            if final_processed_cast is None:
                ...
                # 标题和简介翻译 (仅在完整模式下执行) 
                if self.ai_translator and self.config.get(constants.CONFIG_OPTION_AI_TRANSLATE_TITLE_OVERVIEW, False):
                    logger.info(f"  ➜ [完整模式] 正在检查是否需要 AI 翻译标题/简介...")
                    ...
                    # 1. 简介翻译 (后续逻辑使用已更新的 tmdb_details_for_extra 进行判断)
