                    ...
                    try:
                        with get_central_db_connection() as conn:
                            ...
                            if db_row:
                                ...
                                # ★★★ 修复 Strategy B (数据库缓存) 下元数据缺失导致 ID=0 的问题 ★★★
                                # 如果 Step 1 的 API 请求失败了，但数据库里有演员缓存，
                                # 此时 tmdb_details_for_extra 还是骨架 (ID=None/0)。
                                ...
                                if is_metadata_empty and self.tmdb_api_key:
                                    ...
                                    try:
                                        if item_type == "Movie":
                                            ...
                                            if fresh_data_b: tmdb_details_for_extra.update(fresh_data_b)
                                        elif item_type == "Series":
                                            agg_data_b = tmdb.aggregate_full_series_data_from_tmdb(int(tmdb_id), self.tmdb_api_key)
                                            if agg_data_b:
                                                series_details_b = agg_data_b.get("series_details", {})
                                                tmdb_details_for_extra.update(series_details_b)
                                                tmdb_details_for_extra['seasons_details'] = agg_data_b.get('seasons_details', [])
                                                tmdb_details_for_extra['episodes_details'] = agg_data_b.get('episodes_details', {})
                                    except Exception as e_b:
                                        logger.warning(f"  ➜ [快速模式] 补全元数据失败: {e_b}")
