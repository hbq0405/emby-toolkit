...
def get_full_series_data_from_db(tmdb_id: str):
    """
    从数据库获取整部剧集的完整信息（Series + Seasons + Episodes）。
    
    返回:
    1. series_data: 模拟 TMDb API 返回的剧集详情字典
    2. all_episodes: 包含所有分集信息的列表
    3. emby_seasons_state: 本地入库状态字典 {season_num: {ep_num, ...}}
    """
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            ...
            # 构造 series_data 字典 (字段映射到 TMDb 风格)
            series_data = {
                "id": int(series_row['tmdb_id']) if series_row['tmdb_id'].isdigit() else series_row['tmdb_id'],
                "name": series_row['title'],
                "original_name": series_row['original_title'],
                "status": series_row.get('watchlist_tmdb_status'), # TMDb 状态 (Returning Series 等)
                "number_of_episodes": series_row.get('total_episodes'),
                "overview": series_row.get('overview'),
                "poster_path": series_row.get('poster_path'),
                # 注意：数据库存的是 date 对象，转为字符串以兼容现有逻辑
                "first_air_date": str(series_row.get('release_date')) if series_row.get('release_date') else None,
                "seasons": [] 
            }
            ...
            for s_row in season_rows:
                series_data["seasons"].append({
                    "id": int(s_row['tmdb_id']) if s_row['tmdb_id'].isdigit() else s_row['tmdb_id'],
                    "season_number": s_row['season_number'],
                    "episode_count": s_row.get('total_episodes', 0), 
                    "air_date": str(s_row.get('release_date')) if s_row.get('release_date') else None,
                    "poster_path": s_row.get('poster_path'),
                    "overview": s_row.get('overview')
                })
            ...
            all_episodes = []
            emby_seasons_state = {} # {season_num: {ep_num, ...}}

            for ep_row in episode_rows:
                s_num = ep_row['season_number']
                e_num = ep_row['episode_number']
                
                # 构造 episode 字典
                ep_data = {
                    "id": int(ep_row['tmdb_id']) if ep_row['tmdb_id'].isdigit() else ep_row['tmdb_id'],
                    "season_number": s_num,
                    "episode_number": e_num,
                    "name": ep_row['title'],
                    "air_date": str(ep_row.get('release_date')) if ep_row.get('release_date') else None,
                    "overview": ep_row.get('overview'),
                    "still_path": ep_row.get('poster_path') # 数据库复用 poster_path 存剧照
                }
                all_episodes.append(ep_data)

                # 构造本地状态 (in_library=True 的才算)
                if ep_row.get('in_library'):
                    if s_num not in emby_seasons_state:
                        emby_seasons_state[s_num] = set()
                    emby_seasons_state[s_num].add(e_num)

            return series_data, all_episodes, emby_seasons_state

    except Exception as e:
        logger.error(f"DB: 获取剧集 {tmdb_id} 完整数据失败: {e}", exc_info=True)
        return None, [], {}