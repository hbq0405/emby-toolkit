...
def update_subscription_status_by_tmdb_ids(tmdb_ids: List[str], new_status: str):
    """
    【V3 - 新增核心工具】根据 TMDB ID 列表，批量更新 media_metadata 表中的订阅状态。
    """
    if not tmdb_ids:
        return
    ...
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            # 使用 ON CONFLICT 来实现 "INSERT or UPDATE" 的效果
            # 如果记录不存在，就插入一条；如果已存在，就只更新订阅状态。
            sql = """
                INSERT INTO media_metadata (tmdb_id, item_type, subscription_status, first_requested_at)
                VALUES (UNNEST(%s), 'Movie', %s, NOW())
                ON CONFLICT (tmdb_id, item_type) DO UPDATE SET
                    subscription_status = EXCLUDED.subscription_status,
                    first_requested_at = COALESCE(media_metadata.first_requested_at, EXCLUDED.first_requested_at);
            """
            cursor.execute(sql, (tmdb_ids, new_status))
            logger.info(f"成功将 {len(tmdb_ids)} 个媒体项的订阅状态更新为 '{new_status}'。")
    except psycopg2.Error as e:
        logger.error(f"批量更新媒体订阅状态时出错: {e}", exc_info=True)
        raise

def request_media_subscription(media_info: Dict[str, Any], source: Dict[str, Any]):
    """
    请求订阅单个媒体项，并写入完整的元数据。
    这是所有模块触发订阅的标准化函数。

    Args:
        media_info (Dict[str, Any]): 包含媒体完整信息的字典, e.g.,
            {
                "tmdb_id": "123",
                "item_type": "Series",
                "title": "Loki",
        ...
        source (Dict[str, Any]): 描述订阅来源的字典, 
                                 e.g., {"type": "manual", "user": "admin"}
    """
    tmdb_id = media_info.get('tmdb_id')
    item_type = media_info.get('item_type')

    if not all([tmdb_id, item_type, source]):
        logger.warning("请求订阅失败：缺少 tmdb_id, item_type 或 source。")
        return
    ...
    try:
        with get_db_connection() as conn:
            ...
            # SQL语句现在可以处理所有核心元数据，包括 season_number
            sql = """
                INSERT INTO media_metadata (
                    tmdb_id, item_type, subscription_status, subscription_sources_json, 
                    first_requested_at, title, original_title, release_date, poster_path, season_number
                )
                VALUES (
                    %(tmdb_id)s, %(item_type)s, 'WANTED', %(source)s::jsonb,
                    NOW(), %(title)s, %(original_title)s, %(release_date)s, %(poster_path)s, %(season_number)s
                )
                ON CONFLICT (tmdb_id, item_type) DO UPDATE SET
            ...
            cursor.execute(sql, data_to_upsert)
            
            if cursor.rowcount > 0:
                logger.info(f"  ➜ 预缓存成功：媒体 (TMDb ID: {tmdb_id}) 状态已设为 'WANTED' 并写入元数据。来源: {source.get('name')}")

    except psycopg2.Error as e:
        logger.error(f"请求订阅媒体 {tmdb_id} 时发生数据库错误: {e}", exc_info=True)
        raise

def batch_request_media_subscriptions(media_requests: List[Dict[str, Any]], item_type: str):
    """
    批量请求订阅，并写入包括季号在内的完整元数据。
    """
    ...
    try:
        with get_db_connection() as conn:
            ...
            # ▼▼▼ 核心修改 1/2：在SQL语句中增加 season_number 字段 ▼▼▼
            sql = """
                INSERT INTO media_metadata (
                    tmdb_id, item_type, subscription_status, subscription_sources_json, 
                    first_requested_at, title, original_title, release_date, poster_path, season_number
                ...
                ON CONFLICT (tmdb_id, item_type) DO UPDATE SET
                    ...
                    release_date = EXCLUDED.release_date,
                    poster_path = EXCLUDED.poster_path,
                    season_number = EXCLUDED.season_number -- 确保更新时也写入季号
                WHERE 
                    media_metadata.in_library = FALSE;
            """
            
            # ▼▼▼ 核心修改 2/2：在准备数据时，加入 season_number ▼▼▼
            data_to_upsert = []
            for req in media_requests:
                data_to_upsert.append({
                    ...
                    "title": req.get('title'),
                    "original_title": req.get('original_title'),
                    "release_date": req.get('release_date') or None,
                    "poster_path": req.get('poster_path'),
                    "season_number": req.get('season_number') or None # 从请求中获取季号
                })
            ...
            if cursor.rowcount > 0:
                logger.info(f"  ➜ 批量预缓存成功：{cursor.rowcount} 个 '{item_type}' 媒体项的状态已更新为 'WANTED' 并写入元数据。")
            else:
                logger.info(f"  ➜ 批量预缓存完成，没有媒体项的状态需要更新。")

    except psycopg2.Error as e:
        logger.error(f"批量请求订阅媒体时发生数据库错误: {e}", exc_info=True)
        raise

def cancel_media_subscription(tmdb_id: str, item_type: str):
    """
    【V3 - 核心取消订阅入口】取消对单个媒体项的订阅。
    注意：这只是将状态设为 NONE，如果还有其他来源需要它，状态可能会被再次设为 WANTED。
    更复杂的“移除来源”逻辑可以在未来实现。
    """
    try:
        with get_db_connection() as conn:
            ...
            if cursor.rowcount > 0:
                logger.info(f"  ➜ 已取消对媒体 (TMDb ID: {tmdb_id}) 的订阅请求。")
    except psycopg2.Error as e:
        logger.error(f"取消媒体 {tmdb_id} 订阅时发生数据库错误: {e}", exc_info=True)
        raise

def ignore_media_item(tmdb_id: str, item_type: str):
    """
    将单个媒体项的状态设置为 'IGNORED'。
    - 如果媒体项不存在，则创建一条新记录并标记。
    - 如果已存在，则直接更新其状态。
    - 同时会清空订阅来源，确保其状态干净。
    """
    if not tmdb_id or not item_type:
        logger.warning("忽略媒体项失败：缺少 tmdb_id 或 item_type。")
        return

    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            # 使用强大的 ON CONFLICT (UPSERT) 语句，一句话搞定“没有就新增，有了就更新”
            sql = """
                INSERT INTO media_metadata (tmdb_id, item_type, subscription_status, subscription_sources_json)
                VALUES (%s, %s, 'IGNORED', '[]'::jsonb)
                ON CONFLICT (tmdb_id, item_type) DO UPDATE SET
            ...
            if cursor.rowcount > 0:
                logger.info(f"  ➜ 媒体项 (TMDb ID: {tmdb_id}) 状态已成功设置为 'IGNORED'。")

    except psycopg2.Error as e:
        logger.error(f"忽略媒体项 {tmdb_id} 时发生数据库错误: {e}", exc_info=True)
        raise
